...
def VGG_16():
    ...
    for i in ['block1_conv1','block1_conv2','block2_conv1','block2_conv2','block3_conv1',
        ...
        with h5.File('im_weights.h5',mode='r') as f:
            if i !='fc3':
                #x1=f[i+'/'+i+'_W_1:0']
                #x2=f[i+'/'+i+'_b_1:0']
                x1=f['model_1/'+i+'/kernel:0']
                x2=f['model_1/'+i+'/bias:0']
            else:          
                x1=f['model_1/'+i+'/kernel:0']
                x2=f['model_1/'+i+'/bias:0']
...
if __name__=='__main__':   
    ...
    data=Data_load(train_file,test_file,5,128)
    ...
    history=model.fit_generator(generator=data.Generate_TrainData(),
                                steps_per_epoch=int(NUM_SAMPLE/128),
                                epochs=5,
                                callbacks=[reduce_lr])
    '''
