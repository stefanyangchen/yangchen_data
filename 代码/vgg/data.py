...
class Data_load:
    ...
    def image_preprocess(self,img,labels,model):
        ...
        """
        #创建均值张量
        IMAGENET_MEAN =tf.ones((224,224,3))*tf.constant([122.68,116.64,104.00],shape=(1,1,3))
        #裁剪图片
    ...
    def Generate_TestData(self):
        ...
        dataset=dataset.repeat(1).map(self.parser_test,num_parallel_calls=32).batch(100)
        iterator=dataset.make_one_shot_iterator()
        images,labels=iterator.get_next()
...
if __name__=='__main__':
    d=Data_load('F:/Train_Tfrecords','F:/test.tfrecords',10,10)
